    ...
    def handle_msg(self, from_sock):
        ...
        if len(msg) > 0:
            ...
            if msg["action"] == "connect":
                ...
                if self.group.is_member(to_name) and to_name != from_name:
                    self.group.connect(from_name, to_name)
                    to_sock = self.logged_name2sock[to_name]
                    mysend(to_sock, json.dumps({"action": "game", "status": "request", "from": from_name,'pos':positions}))
                    msg = json.dumps({"action": "game", "status": "success"})
                else:
                    msg = json.dumps({"action": "game", "status": "failure"})
                mysend(from_sock, msg)
...
# ==============================================================================
                ...
                to_sock = self.logged_name2sock[to_name]
                mysend(to_sock, json.dumps({"action":"move", "from":msg["from"], "message":msg["message"]}))
